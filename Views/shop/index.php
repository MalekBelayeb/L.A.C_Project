...
<li id="menu-item-99" class="menu-item menu-item-type-post_type menu-item-object-page no_group menu-item-99" data-depth="0"><a href="contact-us/index.html" class=""><span class="menu-title">Contact </span></a></li>
                    ...
                    <h1> <?php  if (isset($_GET['s'])) if (empty($_GET['s'])) echo "Shop"; else echo "search result for ".$_GET['s'];  ?></h1>
                </div>
                        <!-- #page-title -->
	<div id="content" class="site-content">
<div class="book-junky-shop no-sidebar">
    ...
    <select name="product_cat" id="product_cat">
        ...
        <?php
        					$c = connexion::getConnexion();
        						$bks = $c->query("SELECT * FROM category");
        							while ($category = $bks->fetch(PDO::FETCH_ASSOC)) :
        ?>

        <option value="<?php  echo $category['CATEGORY'];  ?>" 	<?php if (isset($_GET['product_cat']))  if ($_GET['product_cat']==$category['CATEGORY']) echo "selected=''"; else echo "";  ?> >   <?php  echo $category['CATEGORY'];  ?>  </option>
        <?php
        endwhile;
        ?>
...
<select name="orderby" class="orderby">
        ...
        <div class="wrap-search">
            ...
            <button type="submit" class="search-submit" name="button_search">Search Books</button>
        </div>
    ...
    <div class="wrap-archive">
        <div class="row">
                    ...
                    <div class="sidebar-filter col-xs-12 col-md-4 col-lg-3">
                        ...
                        <aside id="filter_book-2" class="widget-woo widget_filter_book">        
            ...
            <div class="bj-ft-rating">
                <h3>Average Rating</h3>
                            ...
                            <input type="checkbox" class="bj-ft-input-rating" id="5-star"
                                   value="5" <?php  if (isset($_GET['bj_meta__wc_average_rating'])) if (strpos($_GET['bj_meta__wc_average_rating'], '5') !== false) echo "checked=''"; else echo "";  ?> ><label
                                    for="5-star">5 star</label>
                        </div>
                                                <div class="bj-ft-rating-item">
                            <input type="checkbox" class="bj-ft-input-rating" id="4-star"
                                   value="4" <?php  if (isset($_GET['bj_meta__wc_average_rating'])) if (strpos($_GET['bj_meta__wc_average_rating'], '4') !== false) echo "checked=''"; else echo "";  ?>><label
                                    for="4-star">4 star</label>
                        </div>
                                                <div class="bj-ft-rating-item">
                            <input type="checkbox" class="bj-ft-input-rating" id="3-star"
                                   value="3" <?php  if (isset($_GET['bj_meta__wc_average_rating'])) if (strpos($_GET['bj_meta__wc_average_rating'], '3') !== false) echo "checked=''"; else echo "";  ?> ><label
                                    for="3-star">3 star</label>
                        </div>
                                                <div class="bj-ft-rating-item">
                            <input type="checkbox" class="bj-ft-input-rating" id="2-star"
                                   value="2" <?php  if (isset($_GET['bj_meta__wc_average_rating'])) if (strpos($_GET['bj_meta__wc_average_rating'], '2') !== false) echo "checked=''"; else echo "";  ?> ><label
                                    for="2-star">2 star</label>
                        </div>
                                                <div class="bj-ft-rating-item">
                            <input type="checkbox" class="bj-ft-input-rating" id="1-star"
                                   value="1" <?php  if (isset($_GET['bj_meta__wc_average_rating'])) if (strpos($_GET['bj_meta__wc_average_rating'], '1') !== false) echo "checked=''"; else echo "";  ?> ><label
                                    for="1-star">1 star</label>
                        </div>
                                                <div class="bj-ft-rating-item">
                            <input type="checkbox" class="bj-ft-input-rating" id="0-star"
                                   value="0" <?php  if (isset($_GET['bj_meta__wc_average_rating'])) if (strpos($_GET['bj_meta__wc_average_rating'], '0') !== false) echo "checked=''"; else echo "";  ?> ><label
                ...
                <button type="submit"
                        class="bj-ft-price-button" name="button_search">Filter</button>
...
<?php
				...
				$book->Maj_Rating();
					...
					{
						$search_barre = $_GET['s'];
						$search_barre = preg_replace("#[^0-9^a-z]#i","",$_GET['s']);
					}else $search_barre ="";

							
					
							if ($_GET['bj_meta__wc_average_rating']=="0")
								$test = "0";
							else if (empty($_GET['bj_meta__wc_average_rating']) )
								$test = "5,4,3,2,1,0";
							else
							$test  = $_GET['bj_meta__wc_average_rating'];




								
						$rows = $book->Nbr_Livre($_GET['product_cat'],$test,$_GET['min_price'],$_GET['max_price'],$search_barre);
...
?>
	 ...
	 <?php  echo $rows['NbNews'];?> BOOKS FOUND</p>
          ...
          <?php
           		      	...
           		      	$sort = $book->Sort_Livre($_GET['orderby']);

           		      	
           		      	$bks = $book->Get_Livre_Recherche($_GET['product_cat'],$test,$_GET['min_price'],$_GET['max_price'],$search_barre,$limit,$sort);
...
$paginationCTR = '';
...

	{
		...
		{
			$previous = $pagenum - 1;
			$paginationCTR .= "<a class='prev page-numbers' href='".$_SERVER['PHP_SELF']."?pn=".$previous."&s=".$search_barre."&product_cat=".$_GET['product_cat']."&bj_tax_pa_year_publication=&post_type=product&bj_action=bj_product&bj_meta__wc_average_rating=".$_GET['bj_meta__wc_average_rating']."&min_price=".$_GET['min_price']."&max_price=".$_GET['max_price']."&orderby=".$_GET['orderby']."'><i class='fa fa-angle-left'></i> Previous Page</a>";
		
			for ($i=$pagenum-4; $i <$pagenum ; $i++) { 
				if ($i >0)
				{
					$paginationCTR .="<a class='page-numbers' href='".$_SERVER['PHP_SELF']."?pn=".$i."&s=".$search_barre."&product_cat=".$_GET['product_cat']."&bj_tax_pa_year_publication=&post_type=product&bj_action=bj_product&bj_meta__wc_average_rating=".$_GET['bj_meta__wc_average_rating']."&min_price=".$_GET['min_price']."&max_price=".$_GET['max_price']."&orderby=".$_GET['orderby']."' >".$i."</a>";
				}
			}
		...
		{
			$next = $pagenum +1;
			$paginationCTR .= "<a class='next page-numbers' href='".$_SERVER['PHP_SELF']."?pn=".$next."&s=".$search_barre."&product_cat=".$_GET['product_cat']."&bj_tax_pa_year_publication==&post_type=product&bj_action=bj_product&bj_meta__wc_average_rating=".$_GET['bj_meta__wc_average_rating']."&min_price=".$_GET['min_price']."&max_price=".$_GET['max_price']."&orderby=".$_GET['orderby']."'>Next Page <i class='fa fa-angle-right'></i></a>";
		}
	}
...
?>                                                                                                                            
                                ...
                                <div class="book-junky-product clearfix col-xs-12 col-sm-12 col-md-12 col-lg-6 post-340 product type-product status-publish has-post-thumbnail product_cat-comedy product_tag-boring-girls product_tag-boring-girls-a-novel product_tag-sara-taylor  instock sale taxable shipping-taxable purchasable product-type-simple">
	<div class="product-thumbnial" style="transition:all 0.25s ease 0s ;box-shadow: 0 0 15px -2px #00576c;" onmouseover="this.style.boxShadow ='0 0 20px 0 #00576c';" onmouseout="this.style.boxShadow ='0 0 15px -2px #00576c';" >
		<a href="shattered/profil.php?id_book=<?php echo $product['ID'] ?>">
...
?>	
	...
	<span class="book-junky-sale">Sale!</span>
...
<img width="330" height="500" src="../wp-content/uploads/<?php echo  $product['IMAGE']; ?> " class="attachment-shop_catalog size-shop_catalog wp-post-image" alt="" />		</a>
	</div>

	<div class="product-content">

		<a href="shattered/profil.php?id_book=<?php echo $product['ID'] ?>" class="title-product">
			<?php   echo $product['NOM'];  ?>	</a>

		<div class="author-product"><?php     
						echo $donnes_auteur['NOM'];
...
<!-- Cached page generated by WP-Super-Cache on 2018-02-03 22:20:41 -->
